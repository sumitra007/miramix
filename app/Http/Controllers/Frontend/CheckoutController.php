...
if($data['payment_status'] =='Completed')
{
    ...
    public function success()
    {
		...
		if($data['payment_status'] =='Completed')
		{
			@mail("sumitra.unified@gmail.com", "update From Success page ", "Invalid Response<br />data = <pre>".print_r($data, true)."</pre>");
			$order_id= $cnt[1];
							$user_id= $cnt[0];
...
//echo "ss= ".$admin_users_email; exit;
            ...
            $user_email = "sumitra.unified@gmail.com";//$user_details->email;
            
            //echo $resetpassword_link; exit;

            /* Mail For Member  */
            $sent = Mail::send('frontend.checkout.order_details_mail', array('receiver_name'=>$mailing_name,'email'=>$user_email,'order_list'=>$order_list), 
            ...
            {
				if(!in_array($each_order_list->brand_id,$branduser))
				{
					$brand_name  = $each_order_list->brand_name; 
		            $brand_email = $each_order_list->brand_email;


		            $sent_brand = Mail::send('frontend.checkout.brand_order_details_mail', array('brand_name'=>$brand_name,'brand_email'=>$brand_email,'order_list'=>$order_list), 
		            ...
		            $branduser[] = $each_order_list->brand_id;
				}
            ...
            $sent_admin = Mail::send('frontend.checkout.admin_order_deatils_mail', array('receiver_name'=>$admin_name,'admin_email'=>$admin_email,'order_list'=>$order_list), 
